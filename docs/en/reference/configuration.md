...
On supported platforms, a console app is invoked from the terminal using the app name (for example, `hello-cli`), not the [`formal_name`][]. The existing [CLI apps guide][cli-apps] has a fuller walkthrough, including how this differs from the human-facing names used by installers and bundles.
...
For [console apps][cli-apps], `formal_name` still controls the human-facing name used by installers and packaged artifacts. The executable that users invoke from the terminal continues to use the app name described in [`console_app`][].
