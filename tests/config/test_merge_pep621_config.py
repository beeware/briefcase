...
def test_specified_license_file():
    """The license file is included in the briefcase config if specified in
    the PEP621 config."""
    briefcase_config = {"key": "value"}
...
def test_no_test_dependencies():
    """If the global config doesn't specify test dependencies, test_requires
    are used as is."""
...
def test_test_dependencies_without_requires():
    """If the global config doesn't specify test requirements, test
