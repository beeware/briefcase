...
def test_app_name_is_formatted(convert_command, monkeypatch):
    ...
    assert mock_input_text.call_args.kwargs["variable"] == "formal name"
    assert mock_input_text.call_args.kwargs["default"] == "Test App Name"
    assert mock_input_text.call_args.kwargs["override_value"] is None
