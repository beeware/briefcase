...
def test_pep621_author(convert_command, monkeypatch):
    ...
    assert mock_input_text.call_args.kwargs["default"] == "mail3@tld.com"
    assert "the selected author name" in mock_input_text.call_args.kwargs["intro"]
...
def test_pep621_wrong_author(convert_command, monkeypatch):
    ...
    mock_input_text.assert_called_once()
    assert mock_input_text.call_args.kwargs["default"] == "noname@bundle.some"
    assert "the author name and bundle" in mock_input_text.call_args.kwargs["intro"]
