...
def test_create_single_with_overrides(tracking_create_command, tmp_path):
    """Command line overrides can be used during app creation."""
    tracking_create_command(
        app=tracking_create_command.apps["first"],
        config_overrides=["template='https://example.com/override'"],
    )
    ...
    # The command line override has been applied.
    assert (
        tracking_create_command.apps["first"].template == "https://example.com/override"
    )
