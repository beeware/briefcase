...
def test_app_packages_prelocked_requires(
    create_command,
    myapp,
    app_packages_path,
    app_packages_path_index,
    ...
    (create_command.base_path / myapp.requires_lock).write_text(
        "first==1.1.1\n" "second==1.2.3\n" "third==4.0.0\n"
    )

    create_command.install_app_requirements(myapp, test_mode=False, relock=False)
