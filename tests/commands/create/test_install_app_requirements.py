...
def test_bad_path_index(create_command, myapp, bundle_path, app_requirements_path):
    ...
    with pytest.raises(
        ...
        match=r"Application path index file does not define "
        r"`app_requirements_path` or `app_packages_path`",
    ):
...
def test_app_packages_valid_requires_no_support_package(
    ...
    # Override the cache of paths to specify an app packages path, but no
...
def test_app_requirements_installer_args_without_requires_no_template_support(
    create_command,
    myapp,
    app_path,
    app_requirements_path,
...
def test_app_requirements_installer_args_without_requires_with_template_support(
    create_command,
    myapp,
    app_path,
    app_requirements_path,
