...
def test_simple_call(capsys):
    ...
    sub.check_output(['hello', 'world'])

    sub._subprocess.check_output.assert_called_with(['hello', 'world'])
...
def test_simple_call_with_arg(capsys):
    ...
    sub.check_output(['hello', 'world'], universal_newlines=True)

    sub._subprocess.check_output.assert_called_with(
        ['hello', 'world'],
...
def test_simple_call_with_path_arg(capsys, tmp_path):
    ...
    sub.check_output(['hello', tmp_path / 'location'], cwd=tmp_path / 'cwd')
