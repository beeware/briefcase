...
def test_short_circuit(mock_tools, first_app_config):
    ...
    tool = NativeAppContext.verify(mock_tools, app=first_app_config)
...
def test_unsupported_os(mock_tools, first_app_config):
    ...
    with pytest.raises(
        ...
        NativeAppContext.verify(mock_tools, app=first_app_config)


def test_verify(mock_tools, first_app_config):
    """Subprocess verify always returns/sets subprocess tool."""
    native_app_context_tool = NativeAppContext.verify(mock_tools, app=first_app_config)
