...
def test_cmdline_tools_url(mock_tools, host_os, name):
    """Validate that the SDK URL is computed using `host_os`."""
    mock_tools.host_os = host_os

    assert mock_tools.android_sdk.cmdline_tools_url == (
...
def test_sdkmanager_path(mock_tools, host_os, sdkmanager_name):
    ...
    assert mock_tools.android_sdk.sdkmanager_path == (
...
def test_adb_path(mock_tools, host_os, adb_name):
    """Validate that if the user is on Windows, we run `adb.bat`, otherwise,
    `adb`."""
    # Mock out `host_os` so we can test Windows when not on Windows.
    mock_tools.host_os = host_os

    assert mock_tools.android_sdk.adb_path == (
        mock_tools.android_sdk.root_path / "platform-tools" / adb_name
    )
...
def test_avdmanager_path(mock_tools, host_os, avdmanager_name):
    """Validate that if the user is on Windows, we run `avdmanager.bat`,
    otherwise, `avdmanager`."""
    # Mock out `host_os` so we can test Windows when not on Windows.
    mock_tools.host_os = host_os

    assert mock_tools.android_sdk.avdmanager_path == (
        mock_tools.android_sdk.root_path
        / "cmdline-tools"
        / "latest"
        / "bin"
...
def test_emulator_path(mock_tools, host_os, emulator_name):
    """Validate that if the user is on Windows, we run `emulator.bat`,
    otherwise, `emulator`."""
    # Mock out `host_os` so we can test Windows when not on Windows.
    mock_tools.host_os = host_os

    assert mock_tools.android_sdk.emulator_path == (
        mock_tools.android_sdk.root_path / "emulator" / emulator_name
    )


def test_avd_path(mock_tools, tmp_path):
    assert mock_tools.android_sdk.avd_path == tmp_path / "home" / ".android" / "avd"


def test_simple_env(mock_tools, tmp_path):
    """The SDK Environment can be constructed."""
    assert mock_tools.android_sdk.env == {
...
def test_managed_install(mock_tools):
    ...
    assert mock_tools.android_sdk.managed_install
...
def test_emulator_abi(mock_tools, host_os, host_arch, emulator_abi):
    ...
    assert mock_tools.android_sdk.emulator_abi == emulator_abi
...
def test_bad_emulator_abi(mock_tools, host_os, host_arch):
    ...
    with pytest.raises(
        ...
        mock_tools.android_sdk.emulator_abi
