...
def test_verify_license_passes_quickly_if_license_present(mock_tools, android_sdk):
    """Validate that verify_license() successfully does nothing in its happy path.
...
def test_verify_license_prompts_for_licenses_and_exits_if_you_agree(
    ...
    """Validate that if verify_license() succeeds if you agree to the Android SDK
    license."""
...
def test_verify_license_handles_sdkmanager_crash(mock_tools, android_sdk):
    """Validate that if verify_license() raises a briefcase exception if it fails to
    launch `sdkmanager`."""
...
def test_verify_license_insists_on_agreement(mock_tools, android_sdk):
    """Validate that if the user quits `sdkmanager --licenses` without agreeing to the
    license, verify_license() raises an exception."""
