...
def test_verify_license_passes_quickly_if_license_present(mock_tools):
    ...
    """
    license_path = mock_tools.android_sdk.root_path / "licenses" / "android-sdk-license"
    license_path.parent.mkdir(parents=True)
    license_path.touch()

    mock_tools.android_sdk.verify_license()
    mock_tools.subprocess.run.assert_not_called()


def test_verify_license_prompts_for_licenses_and_exits_if_you_agree(mock_tools):
    """Validate that if verify_license() succeeds if you agree to the Android
    SDK license."""

    def accept_license(*args, **kwargs):
        license_dir = mock_tools.android_sdk.root_path / "licenses"
    ...
    mock_tools.subprocess.run.assert_called_once_with(
        [os.fsdecode(mock_tools.android_sdk.sdkmanager_path), "--licenses"],
        env=mock_tools.android_sdk.env,
        check=True,
    )


def test_verify_license_handles_sdkmanager_crash(mock_tools):
...
def test_verify_license_insists_on_agreement(mock_tools):
