...
def test_missing_avd_config(mock_tools):
    """If an AVD config can't be found, raise an error."""
    with pytest.raises(
        BriefcaseCommandError,
        match=r"Unable to read configuration of AVD @unknownDevice",
    ):
        mock_tools.android_sdk.verify_avd("unknownDevice")


def test_minimal_config(mock_tools, capsys):
    ...
    mock_tools.android_sdk.avd_config = mock.MagicMock(
        return_value={
    ...
    mock_tools.android_sdk.verify_system_image = mock.MagicMock()
    mock_tools.android_sdk.verify_emulator_skin = mock.MagicMock()

    # Verify the AVD
    mock_tools.android_sdk.verify_avd("minimalDevice")

    # The AVD config was loaded.
    mock_tools.android_sdk.avd_config.assert_called_with("minimalDevice")
    ...
    mock_tools.android_sdk.verify_system_image.assert_not_called()

    # An emulator skin was not verified
    mock_tools.android_sdk.verify_emulator_skin.assert_not_called()


def test_valid_system_image(mock_tools):
    ...
    # Verify the AVD
    mock_tools.android_sdk.verify_avd("goodDevice")

    # The AVD config was loaded.
    mock_tools.android_sdk.avd_config.assert_called_with("goodDevice")
...
def test_valid_emulator_skin(mock_tools):
    ...
    mock_tools.android_sdk.verify_emulator_skin.assert_called_with("pixel_3a")


def test_unrecognized_emulator_skin(mock_tools, capsys):
