...
def test_succeeds_immediately_if_emulator_installed(mock_tools):
    """`verify_emulator()` exits early if the emulator exists in its
    root_path."""
    # Create `emulator` within `root_path`.
    create_emulator(mock_tools.android_sdk.root_path)

    # Also create the platforms folder
    (mock_tools.android_sdk.root_path / "platforms").mkdir(parents=True)

    mock_tools.android_sdk.verify_emulator()

    # Platforms folder still exists
    assert (mock_tools.android_sdk.root_path / "platforms").exists()
...
def test_creates_platforms_folder(mock_tools):
...
def test_installs_android_emulator(mock_tools):
...
def test_install_problems_are_reported(mock_tools):
