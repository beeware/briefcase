...
from briefcase.integrations.android_sdk import ADB, AndroidDeviceNotAuthorized
from briefcase.integrations.base import ToolCache


@pytest.fixture
def mock_tools(tmp_path, mock_tools) -> ToolCache:
    mock_tools.android_sdk.devices = MagicMock(
        return_value={
            "041234567892009a": {
    ...
    mock_tools.android_sdk.adb = mock_adb

    return mock_tools


def test_explicit_device(mock_tools):
    """If the user explicitly names a physical device, it is returned."""

    # Select device with an explicit device ID
    device, name, avd = mock_tools.android_sdk.select_target_device("KABCDABCDA1513")
...
def test_explicit_unauthorized_device(mock_tools):
    ...
    with pytest.raises(AndroidDeviceNotAuthorized):
        mock_tools.android_sdk.select_target_device("041234567892009a")

    # No input was requested
    assert mock_tools.input.prompts == []


def test_explicit_running_emulator_by_id(mock_tools):
    """If the user explicitly names a running emulator by device ID, it is
    selected."""

    # Select emulator with an explicit device ID
    device, name, avd = mock_tools.android_sdk.select_target_device("emulator-5554")
...
def test_explicit_running_emulator_by_avd(mock_tools):
    """If the user explicitly names a running emulator by AVD, it is
    selected."""

    # Select emulator with an explicit device ID
    device, name, avd = mock_tools.android_sdk.select_target_device("@runningEmulator")
...
def test_explicit_idle_emulator(mock_tools):
    """If the user explicitly names an idle emulator by AVD, it is selected."""

    # Select emulator with an explicit device ID
    device, name, avd = mock_tools.android_sdk.select_target_device("@idleEmulator")
...
def test_explicit_invalid_device(mock_tools):
    """If the user explicitly names a non-existet device, an error is
    raised."""

    # Select emulator with an invalid device ID
    with pytest.raises(InvalidDeviceError):
        device, name, avd = mock_tools.android_sdk.select_target_device("deadbeefcafe")

    # No input was requested
    assert mock_tools.input.prompts == []


def test_explicit_invalid_avd(mock_tools):
    """If the user explicitly names a non-existent device, an error is
    raised."""

    # Select emulator with an invalid AVD
    with pytest.raises(InvalidDeviceError):
        device, name, avd = mock_tools.android_sdk.select_target_device(
            "@invalidEmulator"
        )
...
def test_select_device(mock_tools, capsys):
    ...
    device, name, avd = mock_tools.android_sdk.select_target_device(None)
...
def test_select_unauthorized_device(mock_tools):
    ...
    # Run the selection with no pre-existing choice
    with pytest.raises(AndroidDeviceNotAuthorized):
        mock_tools.android_sdk.select_target_device(None)


def test_select_running_emulator(mock_tools, capsys):
...
def test_select_idle_emulator(mock_tools, capsys):
...
def test_select_create_emulator(mock_tools, capsys):
    """If the user manually selects a running device, details are returned."""
    # Mock the user input
...
def test_input_disabled(mock_tools):
...
def test_input_disabled_no_simulators(mock_tools):
    """If input has been disabled, and there are no simulators, 'create' is
    selected."""
    mock_tools.input.enabled = False

    # Remove all the devices and emulators
    mock_tools.android_sdk.devices = MagicMock(return_value={})
    mock_tools.android_sdk.emulators = MagicMock(return_value=[])
...
def test_input_disabled_one_device(mock_tools):
