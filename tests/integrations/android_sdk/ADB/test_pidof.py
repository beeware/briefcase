...
def test_pidof_succeed(mock_tools):
    ...
    adb.run.assert_called_once_with(
        "shell", "pidof", "-s", "com.example", stealth=False
    )


def test_pidof_stealth(mock_tools):
    "adb.pidof() can be called in stealth mode on an process that exists."
    adb = ADB(mock_tools, "exampleDevice")
    adb.run = Mock(return_value="5678\n")
    assert adb.pidof("com.example", stealth=True) == "5678"
    adb.run.assert_called_once_with("shell", "pidof", "-s", "com.example", stealth=True)
