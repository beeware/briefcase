...
def test_vswhere_non_list_content(mock_tools, vswhere_path):
    ...
    raised."""
    # MSBuild is not on the path, and vswhere returns JSON content,
    # but not in the format expected
    mock_tools.subprocess.check_output.side_effect = [
        FileNotFoundError,  # MSBuild not on path
...
def test_vswhere_msbuild_not_installed(mock_tools, tmp_path, vswhere_path):
    ...
    error is raised."""
    # MSBuild is not on the path; vswhere a valid location,
    # but there's no MSBuild there.
    mock_tools.subprocess.check_output.side_effect = [
        FileNotFoundError,  # MSBuild not on path
        json.dumps(
            [
