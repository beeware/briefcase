...
def test_open_docker(open_command, first_app_config, tmp_path):
    ...
    open_command._subprocess.run.assert_called_once_with(
        [
            ...
            "briefcase/com.example.first-app:py3.10",
            "/bin/bash",
        ],
        stream_output=False,
    )
...
def test_open_docker_with_mounts(open_command, first_app_config, tmp_path):
    """Extra mounts will be added to the docker session if there are local
    requirements."""

    first_app_config.requires.append("/path/to/local")
    first_app_config.test_requires.append("/path/to/test_local")
    ...
    open_command._subprocess.run.assert_called_once_with(
        [
            ...
            f"{open_command.data_path}:/home/brutus/.cache/briefcase:z",
            "--volume",
            "/path/to/local:/requirements/local:z",
            "--volume",
            "/path/to/test_local:/requirements/test_local:z",
            "briefcase/com.example.first-app:py3.10",
            "/bin/bash",
        ],
        stream_output=False,
