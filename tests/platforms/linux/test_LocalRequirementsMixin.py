...
def test_install_app_requirements_in_docker(create_command, first_app_config, tmp_path):
    ...
    create_command.tools.subprocess.run.assert_called_once_with(
        [
            ...
            "--rm",
            "--volume",
            f"{tmp_path / 'base_path' / 'dummy'}:/app:z",
            "--volume",
            ...
            "--upgrade",
            "--no-user",
            "--target=/app/weird/First App/path/to/app_packages",
            "foo==1.2.3",
            "bar>=4.5",
        ],
        check=True,
...
def test_install_app_requirements_no_docker(
    ...
    no_docker_create_command.tools[
        ...
        [
            ...
            "--upgrade",
            "--no-user",
            f"--target={tmp_path}/base_path/dummy/weird/First App/path/to/app_packages",
            "foo==1.2.3",
            "bar>=4.5",
        ],
        check=True,
...
def test_install_app_requirements_with_locals(
    ...
    create_command.tools.subprocess.check_output.assert_called_once_with(
        [
            ...
            "--sdist",
            "--outdir",
            tmp_path / "base_path" / "dummy" / "weird" / "First App" / "_requirements",
    ...
    create_command.tools.subprocess.run.assert_called_once_with(
        [
            ...
            "foo==1.2.3",
            "bar>=4.5",
            "/app/weird/First App/_requirements/first-1.2.3.tar.gz",
            "/app/weird/First App/_requirements/second-2.3.4.tar.gz",
            "/app/weird/First App/_requirements/third-3.4.5-py3-none-any.whl",
        ],
        check=True,
    )
